...
* `development-brian-526` - Plugin Human-readable name, e.g. "Sample Extension"
* `Developmentbrian526` - Which namespace we will use for the plugin. Usually the plugin name or the slug in uppercase without spaces, e.g. "SampleExtension"
* `development-brian-526` - Uses using dashes normally, e.g. "sample-extension"
* `__TRIBE_DOMAIN__` - Translation domain normally with "tribe-" prefix, e.g. "tec-labs-sample-extension"
* `development_brian_526` - Uses underscores, so it's safe for variables, e.g. "sample_extension"
